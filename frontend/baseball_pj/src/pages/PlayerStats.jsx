            ...
            {players.map((p, idx) => (
              <tr key={p.playerId || p.id || idx}>
                ...
                {type === 'hitter' ? (
                  ...
                  <>
                    <td>{p.era}</td><td>{p.gamesPlayed}</td><td>{p.wins}</td><td>{p.losses}</td>
                    <td>{p.saves}</td><td>{p.inningsPitched}</td><td>{p.hitsAllowed}</td>
                    <td>{p.homeRunsAllowed}</td><td>{p.walksAllowed}</td><td>{p.strikeouts}</td>
                  </>
                )}
              </tr>
            ))}
